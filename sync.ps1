...
if ($Push) {
    ...
    # ── بوّابة السلاج ─────────────────────────────────────────────
    # تحويلُ الاسم المستعار يُكتب في الجذر بسلاج الاسم، فإن ساوى سلاجَ صفحةٍ
    # حلّ محلّها: 254 بايتًا مكان الفصل، أو تحويلٌ إلى نفسه لا ينتهي. ولا يظهر
    # ذلك في أوبسيديان بحال، لأنّ روابطه بالأسماء لا بالمسارات. (CLAUDE.md 15/ح)
    Write-Host "→ فحص تصادم مسارات النشر..." -ForegroundColor Cyan
    # ‎--slug-roots self‎ لازم: المحتوى المولَّد **هو** جذر الموقع، ومجلداته ليست
    # جذور نشر. وبلا التصريح تُقاس صفحات المجلدات الفرعية على مجلداتها فتُعدّ
    # أسماؤها المستعارة حاجبةً وهي اختصاراتٌ نافعة. (CLAUDE.md 15/ح)
    python $Audit --root $Content --only slugs --glossary= --slug-roots self
    if ($LASTEXITCODE -ne 0) {
